...
class MazeSolver:
    ...
    def search(self, i, j, destination_x, destination_y):
        ...
        # why 0? of course because in the fist iteration the min_distance is 0
        queue.append((i, j, 0))

        while queue:

            # we take first item we have inserted
            (i, j, dist) = queue.popleft()

            # if we have reched the destination - we break out of the while loop
            # because we have found the destination
...
if __name__ == "__main__":
    ...
    maze_sorver = MazeSolver(m)
    maze_sorver.search(0, 0, 4, 4)
    maze_sorver.show_result()